...
`/products` (canFullEdit). 2 ส่วนต่อรุ่น:
...
ตอนสร้างงาน/อนุมัติเลือกรุ่น → prefill `item` + copy BOM (`JobMaterial`) + ชุดประกอบ (`JobAssembly`). ไม่มี FK กลับ (copy ตอนเลือกเท่านั้น). ลบรุ่นไม่กระทบงาน.
...
> **Schema ที่ push prod แล้ว (2026-05-27):** `Shipment`, `Job.shippedAt`, `ProductAssembly`, `JobAssembly` (additive).
