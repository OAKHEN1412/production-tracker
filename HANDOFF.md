...
- **ระยะเผื่อตัด (`Product.cutAllowanceMm`, 1 ค่า/รุ่น)** — บวกเข้า `cutLengthMm` ของวัสดุเส้นทุกตัว (cutLengthMm>0) ตอน copy ไปงาน (kerf/เผื่อปาด). recipe เก็บค่าบริสุทธิ์; bake ฝั่ง client (`withAllowance` ใน JobForm/ApprovalsView + DraftFields) ตอนเลือกรุ่น. วัสดุนับชิ้นไม่กระทบ.

ตอนสร้างงาน/อนุมัติเลือกรุ่น → prefill `item` + copy BOM (`JobMaterial`, บวกเผื่อตัด) + ชุดประกอบ (`JobAssembly`). ไม่มี FK กลับ (copy ตอนเลือกเท่านั้น). ลบรุ่นไม่กระทบงาน.
...
Product       id, code (unique?), name (unique), notes, cutAllowanceMm  # รุ่นกระบอก (cutAllowanceMm = เผื่อตัด mm บวกเข้าวัสดุเส้น)
...
- ~~**ระยะเผื่อตัดวัสดุ (ต่อรุ่น)**~~ ✅ `Product.cutAllowanceMm` บวกเข้าความยาวตัดวัสดุเส้นตอนใช้รุ่น.

> **Schema ที่ push prod แล้ว (2026-05-27):** `Shipment`, `Job.shippedAt`, `ProductAssembly`, `JobAssembly`, `Product.cutAllowanceMm` (additive).
