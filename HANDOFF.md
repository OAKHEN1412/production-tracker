...
| **SHIPPING** | รับเข้าคลัง + จัดการสต๊อกวัสดุ + **ยืนยันส่งออก** (`/shipping`) (ไม่ยุ่งงานผลิต/recipe/user) |
| **SALES** | read-only |
...
`/shipping` (canShip = OWNER/PRODUCTION/SHIPPING). ยืนยันการส่งของออกให้ลูกค้า.
- **tab "รอจัดส่ง"** = งาน status `DONE` (ผลิตเสร็จ, ยังไม่ส่ง) → กดยืนยัน → upload รูป (compress client-side เหมือนรับเข้า) + หมายเหตุ (เลขพัสดุ/ขนส่ง) → `POST /api/shipments`.
- ยืนยันแล้ว: สร้าง `Shipment` (1 ต่อ job, `jobId` unique) + job → status `SHIPPED` + `shippedAt`. ส่งซ้ำ → 409. ส่งงานที่ยังไม่ DONE → 400.
- **tab "ส่งแล้ว"** = ประวัติ shipment พร้อมรูป (serve ผ่าน `/api/shipments/[id]/photo`, lazy + cache immutable).
- SHIPPED นับเป็น "เสร็จเดือนนี้" รวมกับ DONE ใน stats (ใช้ `finishedAt`).
...
Job       id, seq (unique), docNo (unique), orderDate, deliveryTime (auto),
          ...
          rate, etaAuto (auto), etaManual, startedAt, finishedAt, shippedAt,
          materialsDeducted (bool),
          assignedToId, salesOwnerId, createdById → User
Shipment  id, jobId (unique → Job), photo (base64), note, shippedAt, createdById   # outbound confirm
...
**Status** (string): WAITING_APPROVAL / PENDING / IN_PROGRESS / PAUSED / QC / DONE / **SHIPPED** / CANCELLED — label/สีใน `src/lib/eta.ts`. (string column → เพิ่ม status ใหม่ไม่ต้อง migrate). `SHIPPED` = DONE แล้วยืนยันส่งออกที่ `/shipping`.
...
src/
  ...
  components/
    JobTable, JobForm, MultiJobForm, EtaPopup, StatsSidebar, UploadExcel,
...
- ~~**คิว/สต๊อกจัดส่ง**~~ ✅ คิว "รอจัดส่ง" = งาน status **DONE** (ผลิตเสร็จ) ที่ยังไม่ SHIPPED. หน้า `/shipping` tab "รอจัดส่ง". *(หมายเหตุ: trigger ใช้ DONE ไม่ใช่ approve — ของต้องผลิตเสร็จก่อนถึงส่งได้)*
- ~~**หน้า SHIPPING ยืนยันส่งออก (outbound)**~~ ✅ `/shipping` กดยืนยัน → upload รูป (compress client-side) + หมายเหตุ → job status `SHIPPED` + `shippedAt`, สร้าง `Shipment` row. tab "ส่งแล้ว" = history พร้อมรูป.
- ~~**SUPPORT ใส่ ETA manual**~~ ✅ SUPPORT ใส่ "วันที่ต้องการ" (etaManual) ในฟอร์มคำขอได้ (JobForm + dashboard inline). คง survive ผ่าน approval.
- ~~**สั่งงานหลายรายการพร้อมกัน**~~ ✅ ปุ่ม "+ หลายงาน" บน dashboard → `MultiJobForm` หลายแถว (แต่ละแถว = 1 job แยก seq) → POST `/api/jobs/bulk`. SUPPORT → ทุกแถวเป็น WAITING_APPROVAL.

> ⚠️ **ต้อง `npx prisma db push` ชี้ prod ก่อน deploy** — เพิ่ม table `Shipment` + column `Job.shippedAt` (additive ปลอดภัย). ยังไม่ push.
