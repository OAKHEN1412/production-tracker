...
## Shipping (มาส่งของเข้าโรงงาน — inbound gate ก่อนผลิต)

`/shipping` (canShip = OWNER/PRODUCTION/SHIPPING). **ไม่เกี่ยวกับสต๊อกวัสดุ** — เป็นการยืนยันว่าฝ่ายจัดส่งเอาของ/ชุดประกอบมาส่งที่โรงงานแล้ว เพื่อปล่อยงานเข้าผลิต.
- **tab "รอจัดส่ง"** = งาน status `AWAITING_DELIVERY` (PRODUCTION อนุมัติแล้ว). การ์ดโชว์ **ชุดประกอบ (JobAssembly) × จำนวนผลิต** ที่ต้องเอามาส่ง — ไม่โชว์รายการวัสดุ. กดยืนยัน → upload รูป (compress เหมือนรับเข้า) + พิมพ์ "มาส่งอุปกรณ์อะไร" → `POST /api/shipments`.
- ยืนยันแล้ว: สร้าง `Shipment` (1 ต่อ job, `jobId` unique) + job → status `PENDING` (รอผลิต) + `shippedAt`. (สต๊อก/ETA ตัด+คิดไปแล้วตอน approve — ไม่ต้อง requeue). ยืนยันงานที่ไม่ใช่ `AWAITING_DELIVERY` → 400.
- **tab "มาส่งแล้ว"** = ประวัติ shipment พร้อมรูป (serve ผ่าน `/api/shipments/[id]/photo`, lazy + cache immutable).

---

## Products (recipe รุ่นกระบอก)

`/products` (canFullEdit). ต่อรุ่น:
- **วัสดุ (BOM, `ProductMaterial`)** — ตัดสต๊อก. วัสดุเส้นใส่ "จำนวนเส้น × ความยาวตัด mm/ตัว".
- **ชุดประกอบ (`ProductAssembly`, `AssemblyEditor`)** — ชื่อชุด + จำนวน/ตัว. ไม่มีสต๊อก. ฝ่ายจัดส่งเห็น (× จำนวนผลิต).
- **ระยะเผื่อตัด — มาสเตอร์ global (`Setting.cutAllowanceMm`, ค่าเดียวทั้งระบบ)** — ตั้งที่กล่องบนสุดหน้า `/products` (canFullEdit, `PATCH /api/settings`). บวกเข้า `cutLengthMm` ของวัสดุเส้นทุกตัว (cutLengthMm>0) ทุกรุ่น (ใหม่/เก่า) ตอน copy ไปงาน (kerf/เผื่อปาด). recipe เก็บค่าบริสุทธิ์; bake ฝั่ง client (`withAllowance` ใน JobForm/ApprovalsView + DraftFields) — page ส่งค่า global ลงเป็น prop `cutAllowanceMm` (จาก `getCutAllowanceMm()` ใน `lib/settings.ts`). วัสดุนับชิ้นไม่กระทบ.

ตอนสร้างงาน/อนุมัติเลือกรุ่น → prefill `item` + copy BOM (`JobMaterial`, บวกเผื่อตัด global) + ชุดประกอบ (`JobAssembly`). ไม่มี FK กลับ (copy ตอนเลือกเท่านั้น). ลบรุ่นไม่กระทบงาน.
...
Job       id, seq (unique), docNo (unique), orderDate, deliveryTime (auto),
          ...
          rate, etaAuto (auto), etaManual, startedAt, finishedAt, shippedAt (=วันยืนยันมาส่ง),
          materialsDeducted (bool),
          assignedToId, salesOwnerId, createdById → User
Shipment  id, jobId (unique → Job), photo (base64), note, shippedAt, createdById   # SHIPPING ยืนยันมาส่งของเข้าโรงงาน
JobLog        id, jobId, status, message, createdAt
JobMaterial   id, jobId, materialId, qtyPerUnit, cutLengthMm   @@unique([jobId, materialId])   # BOM ของงาน (ตัดสต๊อก)
JobAssembly   id, jobId, name, qty                              # ชุดประกอบของงาน (ฝ่ายจัดส่งเห็น, ไม่มีสต๊อก)
...
Setting       id (="global"), cutAllowanceMm                        # มาสเตอร์ตั้งค่า global (เผื่อตัด mm)
ProductMaterial id, productId, materialId, qtyPerUnit, cutLengthMm @@unique([productId, materialId]) # recipe วัสดุ
ProductAssembly id, productId, name, qty                          # recipe ชุดประกอบ (copy → JobAssembly ตอนเลือกรุ่น)
Delivery      id, title, note, photo (base64), qtyReceived, lengthMm, materialId, createdById
```

**Status** (string): WAITING_APPROVAL / **AWAITING_DELIVERY (รอจัดส่ง)** / PENDING / IN_PROGRESS / PAUSED / QC / DONE / CANCELLED — label/สีใน `src/lib/eta.ts`. (string column → เพิ่ม status ใหม่ไม่ต้อง migrate). `AWAITING_DELIVERY` = อนุมัติแล้ว ตัดสต๊อก+เข้าคิวแล้ว รอ SHIPPING ยืนยันมาส่งของ → PENDING. **เพิ่ม/ลบ status: sync `eta.ts STATUSES`+label+color · `JobTable STATUS_ORDER` · `scheduler` notIn (ถ้าไม่เข้าคิว) · `history TERMINAL` · `stats` · `HistoryView`.**
...
src/
  app/
    ...
    api/
      ...
      materials/ (route, [id], [id]/lengths, bulk)
      products/ (route, [id])
...
## Bug audit + fixes (2026-05-27)

Audit หา bug ทั้ง repo. แก้แล้ว 4/5 + เหลือ manual action 1 อย่าง.

| # | บัค | ไฟล์ | สถานะ |
|---|---|---|---|
| 1 | **Security** — หน้า login โชว์ credentials (`owner1234`) บน production สาธารณะ → ใครก็ login เป็น OWNER | `app/login/page.tsx` | ✅ ซ่อน hint ด้วย `NODE_ENV !== "production"` + แก้ hint ที่ผิด (`production/production123` → `chang_tee/worker123`). ⚠️ **ยังต้อง rotate รหัส `owner1234` เอง** (แก้ `seed.ts` แล้ว reseed หรือผ่าน `/admin/users`) |
| 2 | **Data** — เปลี่ยน `unit` วัสดุ (count↔length) ทำ `Material.qty` หลุดจาก `MaterialLength` buckets → recipe ตัดไม่ได้ทั้งที่ stock มี | `api/materials/[id]/route.ts` | ✅ unit ข้าม boundary → rebuild buckets (count→length เก็บ bucket `lengthMm=0`=qty; length→count ลบ buckets) รักษา invariant qty=Σbuckets |
| 3 | **ETA** — ลบ user ที่ถือคิวงาน ไม่ recompute ETA → งานที่หลุดช่าง stale | `api/users/[id]/route.ts` | ✅ เรียก `recomputeWorkerQueues([target.id])` หลัง unlink |
| 4 | **UX** — `removePiecesAtLength` clamp เงียบ → UI โชว์เบิกสำเร็จหลอก | `api/materials/[id]/route.ts` + `MaterialsTable.tsx` | ✅ route คืน `_warning` ถ้าตัดได้ไม่ครบ, UI alert |
| 5 | **Perf** — scheduler persist งาน assigned ซ้ำ 2 รอบ | `lib/scheduler.ts` | ✅ track `persisted` set ข้าม final persist ที่เขียนไปแล้ว |

**TODO ค้าง:** rotate รหัส OWNER (`owner1234`) บน prod — เป็น manual action นอกโค้ด.
...
| งาน | ที่แก้ |
|---|---|
| เพิ่ม status | `lib/eta.ts` STATUSES+label+color · `JobTable` STATUS_ORDER · `scheduler.ts` notIn (ถ้าไม่เข้าคิว) · `history.ts` TERMINAL (ถ้าจบงาน) · `stats.ts` (ถ้านับเสร็จ) · `HistoryView` isFinished · `NEXT_STATUS` (ถ้ามีปุ่มเดินสถานะ) — **อย่าลืม status-aware logic เหล่านี้ ไม่งั้น ETA/ประวัติเพี้ยน** |
...
### ค้างจากที่สั่ง — ✅ ทำครบแล้ว (2026-05-27)

- ~~**คิว/หน้าจัดส่ง + SHIPPING ยืนยัน**~~ ✅ flow ใหม่ (inbound gate): approve → **รอจัดส่ง** (AWAITING_DELIVERY) → SHIPPING ยืนยันมาส่งของ + รูป + พิมพ์อุปกรณ์ ที่ `/shipping` → **รอผลิต**. ไม่เกี่ยวสต๊อกวัสดุ. ดู section "Shipping". *(เลิกใช้ model ขาออก DONE→SHIPPED เดิมแล้ว)*
- ~~**SUPPORT ใส่ ETA**~~ ✅ SUPPORT เลือก dropdown ช่วงส่ง (deliveryTime) + วันที่ (etaManual) — JobForm + dashboard inline. survive ผ่าน approval.
- ~~**สั่งงานหลายรายการ**~~ ✅ ปุ่มเดียว "+ เพิ่มงาน" → ฟอร์มเดียว กด "+ เพิ่มอีกงาน" ได้ (แต่ละ block = 1 job เต็ม คง product/BOM/ชุดประกอบ/ETA) → POST `/api/jobs` ทีละงาน. (เลิกใช้ MultiJobForm/bulk เดิม)
- ~~**ชุดประกอบ (assembly) ส่งให้ฝ่ายจัดส่ง**~~ ✅ รุ่นกระบอกเพิ่มชุดประกอบ → copy → JobAssembly → /shipping เห็น × จำนวนผลิต (ไม่เห็นวัสดุ).
- ~~**ระยะเผื่อตัดวัสดุ (มาสเตอร์)**~~ ✅ `Setting.cutAllowanceMm` ค่าเดียว global บวกเข้าความยาวตัดวัสดุเส้นทุกรุ่น (ตั้งที่ /products).

> **Schema ที่ push prod แล้ว (2026-05-27):** `Shipment`, `Job.shippedAt`, `ProductAssembly`, `JobAssembly`, `Setting` (มาสเตอร์ cutAllowanceMm). *(per-product `Product.cutAllowanceMm` เคยมีสั้นๆ — เปลี่ยนเป็น global, drop column แล้ว)*
